...
// });
document.getElementById('tambahJabatan').addEventListener('submit', function(event) {
    event.preventDefault();
    const formData = new FormData(this);
    tambahJabatan(formData);
});
...
function confirmDelete(uuid) {
    ...
    if (confirm('Apakah Anda yakin ingin menghapus data ini dari pengurus inti?')) {
        fetch(`/pengurus/delete/${uuid}`, {
            method: 'PUT',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
            }
